...
export function calculateInternalEcho(dataset: AuraDataset) {
  // ==========================================
  // FEATURE EXTRACTION
  // ==========================================
  ...
  [...topArtistIds, ...trackArtistIds, ...recentArtistIds].forEach((id) => {
  ...
  const score = calculateScore([
    {
      value: Math.min(repeatedArtists * 10, 100),
      weight: 40,
    },
    {
      value: Math.min(strongestAnchor * 15, 100),
      weight: 35,
    },
    {
      value: Math.min(recurrence * 25, 100),
      weight: 25,
    },
  ]);
